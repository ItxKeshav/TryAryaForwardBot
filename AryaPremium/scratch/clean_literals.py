...
# I'll use a safer approach: replace the hole function body with triple-quoted f-strings where needed.

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Fix _show_story_details
...
# Replacement logic - strictly finding the start/end of functions to avoid partial matches
def safe_replace(content, func_name, new_body):
    pattern = r'async def ' + func_name + r'\(.*?\):.*?(\n(?=async def|def|#|$))'
    return re.sub(pattern, new_body + "\n", content, flags=re.DOTALL)

content = safe_replace(content, "_show_story_details", new_details)
content = safe_replace(content, "_process_start", new_start)
content = safe_replace(content, "_show_upi_payment_screen", new_upi)

# Also fix the f-strings with newlines in _process_callback manually for the ones I injected
# I'll just look for any remaining ones.

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Functions cleaned and string literals fixed.")
