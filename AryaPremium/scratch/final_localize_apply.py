...
new_show_details = r'''async def _show_story_details(client, msg_or_query, story, lang, bot_cfg: dict = None):
...
new_upi_screen = r'''async def _show_upi_payment_screen(client, user_id, story, lang, bot_cfg, upi_id, qr_card, upi_uri, button_url):
...
def find_func_lines(lines, func_name):
    start = -1
    ...
    # Find next function or major comment block
    end = len(lines)
    for i in range(start + 1, len(lines)):
        if lines[i].strip().startswith("async def ") or lines[i].strip().startswith("def ") or "Callback Handler" in lines[i]:
            end = i
            break
    return start, end

def apply_func(lines, name, content):
    s, e = find_func_lines(lines, name)
    if s is not None:
        lines[s:e] = [content + "\n\n"]
    return lines

lines = apply_func(lines, "_process_start", new_process_start)
lines = apply_func(lines, "_show_story_details", new_show_details)
lines = apply_func(lines, "_show_upi_payment_screen", new_upi_screen)

with open(file_path, 'w', encoding='utf-8') as f:
    f.writelines(lines)
print("Applied all localized functions successfully.")
